        ...
        for (int jdx = 0; jdx < this->pieces.size(); jdx++)
        {
            ...
            else if (Board::__num_features > 1)
            {
                ...
                if (jdx == 0)
                {
                    if (this->expression_type == "prefix")
                    {
                        this->derivePrefix(0, this->pieces[jdx].size() - 1, "x0", this->pieces[jdx], grasp);
                    }
                    else //postfix
                    {
                        this->derivePostfix(0, this->pieces[jdx].size() - 1, "x0", this->pieces[jdx], grasp);
                    }
...
//                        puts("hi");
                ...
                else
                {
                    if (this->expression_type == "prefix")
                    {
                        this->derivePrefix(0, this->pieces[jdx].size() - 1, "x1", this->pieces[jdx], grasp);
                    }
                    else //postfix
                    {
                        this->derivePostfix(0, this->pieces[jdx].size() - 1, "x1", this->pieces[jdx], grasp);
                    }
                    if (isZero(expression_evaluator(this->params, this->derivat), sqrt(this->isConstTol))) //Ignore the trivial solution (N-d functions)!
                    {
                        this->MSE_curr = FLT_MAX;
