...
struct Board
{
    ...
    const std::vector<int> n; //depth of RPN/PN trees
...
//                }
                Board::__input_vars.push_back("x0");
...
//    }
    ...
    int __num_consts_diff() const
    {
        ...
        for (int jdx = 0; jdx < diffeq_result.size(); jdx++)
        {
            for (const std::string& token : diffeq_result[jdx])
            {
                if (token.substr(0,5) == "const")
                {
                    count++;
...
//    
    ...
    bool LevenbergMarquardt()
    {
        ...
        float score_before = MSE(expression_evaluator(this->params, this->diffeq_result)) + MSE(expression_evaluator(this->params, this->pieces[0]), Board::data["x1"]);
        ...
        if ((MSE(expression_evaluator(this->params, this->diffeq_result)) + MSE(expression_evaluator(this->params, this->pieces[0]), Board::data["x1"])) < score_before)
        {
            improved = true;
        }
    ...
    float fitFunctionToData()
    {
        ...
        for (int jdx = 0; jdx < this->pieces.size(); jdx++)
        {
            ...
            if (isConstant(expression_eval, sqrt(this->isConstTol))) //Ignore the trivial solution (1-d functions)!
            {
...
//            {
            ...
            float data_loss = loss_func(expression_evaluator(this->params, this->pieces[0]), Board::data["x1"]);
            this->MSE_curr = ((1.0f/score) - 1.0f) + ((1.0f/data_loss) - 1.0f);
            score = 1.0f/this->MSE_curr;
...
//                puts("hi");
            ...
            float data_loss = loss_func(expression_evaluator(this->params, this->pieces[0]), Board::data["x1"]);
            this->MSE_curr += ((1.0f/data_loss) - 1.0f);
            score = 1.0f/this->MSE_curr;
...
//        std::cout << "depth = " << depth << ", complete = " << complete << "\n";
    ...
    void get_indices(std::vector<std::pair<int, int>>& sub_exprs, std::vector<std::string>& individual, int idx)
    {
        ...
        for (size_t k = 0; k < individual.size(); k++)
        {
            ...
            auto [depth, complete] =  ((expression_type == "prefix") ? getPNdepth(individual, idx, start, stop+1, false /*cache*/) : getRPNdepth(individual, idx, start, stop+1));
...
//const0 -> A, const1 -> B, const2 -> C, const3 -> \tilde{tau}
std::vector<std::vector<std::string>> spruce_budworm(Board& x)
{
    ...
    {
        throw std::invalid_argument("Prefix not implemented for spruce_budworm");
    }
    else if (x.expression_type == "postfix")
    {
        //y'
        x.derivePostfix(0, x.pieces[0].size()-1, "x0", x.pieces[0], grasp);
        for (const std::string& i: x.derivat) // y'
        {
            result.push_back(i);
        }
        
        //A y *
        result.push_back("const0"); // A
        for (const std::string& i: x.pieces[0]) // y
        {
            result.push_back(i);
        }
        result.push_back("*"); // *
        ...
        //B y(x0 - const3) *
        result.push_back("const1"); // B
        for (const std::string& i: x.pieces[0]) // y(x0 - const3)
        {
            if (i == "x0")
            {
                result.push_back(i);
                result.push_back("const3");
                result.push_back("-");
            }
            else
            {
        ...
        result.push_back("*"); // *
        result.push_back("~"); // ~
        result.push_back("exp"); // exp
        
        //B y(x0 - const3) * C y(x0 - const3) * ~ exp *
        result.push_back("*"); // *
        //y' A y * + y y * 1 y y * + / + B y(x0 - const3) * C y(x0 - const3) * ~ exp * -
        result.push_back("-"); // -
    }
...
//    auto data = createMeshgridVectors(101, 1, {0.0001f}, {10.0f});
    ...
    Eigen::MatrixXf data(28, 2);

    float values[28][2] =
    {
        {0, 0.06}, {1, 0.07}, {2, 0.41}, {3, 4}, {4, 5}, {5, 56},
        {6, 55}, {7, 52}, {8, 30}, {9, 65}, {10, 12}, {11, 7},
        {12, 20}, {13, 2.5}, {14, 0.4}, {15, 0.3}, {16, 0.9}, {17, 0.35},
        {18, 0.25}, {19, 0.06}, {20, 0.07}, {21, 0.06}, {22, 0.05},
        {23, 0.45}, {24, 2.7}, {25, 2}, {26, 3.3}, {27, 11}
    };
...
//    std::cout<<data << '\n' << (Eigen::VectorXf::Ones(5).array() / Eigen::VectorXf::Zero(5).array()).cos() /*Eigen::VectorXf::Zero(5).array().pow(Eigen::VectorXf::Ones(5).array())*/ << '\n';
    ...
    RandomSearch(spruce_budworm /*differential equation to solve*/, data /*data used to solve differential equation*/, std::vector<int>{7} /*fixed depths of generated solution*/, "postfix" /*expression representation*/, 4 /*num_consts: number of constants in differential equation*/, "LevenbergMarquardt" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "naive_numerical" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, threshold /*threshold for which solutions cannot be constant*/, true /*whether to any of the const tokens from the differential equation in the original expression, though `const_tokens`must be true as well*/);
...
//g++ -Wall -std=c++20 -o PrefixPostfixMultiThreadDiffSimplifySR_Nd_data PrefixPostfixMultiThreadDiffSimplifySR_Nd_data.cpp -O2 -I/opt/homebrew/opt/eigen/include/eigen3 -I/opt/homebrew/opt/eigen/include/eigen3 -I/Users/edwardfinkelstein/LBFGSpp -L/opt/homebrew/Cellar/boost/1.84.0 -I/opt/homebrew/Cellar/boost/1.84.0/include -march=native
...
//Unique expressions = 61710083
//Time spent fitting = 17782.9 seconds
//Best score = 0.000210754, MSE = 4744.86
//Best expression = ((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000))))))
//Best expression (original format) = 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^
//Best diff result = ((((((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))) * ((ln((1.910317 / sech(x0))) * ~((2 / 27.000000))) + (((~((1.910317 * (~(sech(x0)) * tanh(x0)))) / (sech(x0) * sech(x0))) / (1.910317 / sech(x0))) * (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))))) + (0.260159 * ((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))))) + ((((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))) * ((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000))))))) / (1 + (((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))) * ((1.910317 / sech(x0)) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + (x0 * (2 / 27.000000)))))))))) - ((4.004682 * ((1.910317 / sech((x0 - 19.653419))) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + ((x0 - 19.653419) * (2 / 27.000000))))))) * exp(~((1.910317 * ((1.910317 / sech((x0 - 19.653419))) ^ (sech(ln(1)) + (sqrt(2) - (sqrt(1.910317) + ((x0 - 19.653419) * (2 / 27.000000)))))))))))
//Best expression (original format) = 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ 1.910317 x0 sech / ln 2 27.000000 / ~ * 1.910317 x0 sech ~ x0 tanh * * ~ x0 sech x0 sech * / 1.910317 x0 sech / / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + * + * 0.260159 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ * + 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ * 1 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ 1.910317 x0 sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 2 27.000000 / * + - + ^ * + / + 4.004682 1.910317 x0 19.653419 - sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 19.653419 - 2 27.000000 / * + - + ^ * 1.910317 1.910317 x0 19.653419 - sech / 1 ln sech 2 sqrt 1.910317 sqrt x0 19.653419 - 2 27.000000 / * + - + ^ * ~ exp * -
//Best parameters = 0.260159
// 4.00468
// 1.91032
// 19.6534
//
