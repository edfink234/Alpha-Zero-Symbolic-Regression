...
if mu_equals_nu:
    ...
    f = [\
     ...
     0.00229446744874825*mu*nu*(0.154403901923677*nu - 0.704877892754937)*sin(0.674687420950769*mu**2) - 4.67476523567563e-7*mu*r + 0.0600448854189321*mu + 0.0600448854189321*nu + (0.106681258289303 - 0.0136839100170842*nu)*(1.97314771138743 - 0.0976081912541372*mu)*(-1.54561192921054*nu + tanh(mu))*(0.419506141120843*mu - 2.04124479012295*nu + 3.66949702252684)*sin(sech(nu - 0.708105726380382)) + (3.19776335749732 - 1.70279664278474*mu)*(0.083358403299967*mu*nu + 1.61134791597624*nu + 0.0487676515185577) + (-0.0063699223726253*mu - 0.11526854361308)*sin(sin(mu) + sin(nu) + 4.25269718289635*tanh(nu))*tanh(mu*nu*sin(nu))*sech(tanh(mu)) - (-0.0344148089907704*r - 5.17946280592929e-6)*sin(1.84127720372125*r) + (-7.80913334356013e-6*nu*(mu + 3.38114514976531) + 0.392151814554449)*(-0.363580566258292*nu*(-mu - 19.3048947337649)*(mu - 1.86684312845051) + tanh((mu - 46.9619279884798)*sech(nu))) + (nu + sin(nu) - 1.26728555844956)*sech(3.09837101048152*nu) + (-0.000119071284847732*mu + 0.001913595730321*sin((mu + 0.398454383665899)*tanh(mu)) - 0.109291502121555*tanh(6.38907170792931*mu) + 0.108990149853813)*(3.84905339739903*mu + 0.708810898651849*nu*(mu - 0.739261621507221)*sin(mu*nu) + (-0.0692332318548395*mu + sin(nu))*(mu + 141.838272610881)*(nu - 7.24339361562094)*(tanh(nu) - 1.16846989856565)*sin(mu)*tanh(nu) + sin(10.21591831442*nu) + sech(r)) - 0.727655835687045*(-0.165180944464423*nu + 0.108710339559028*(sech(r) - 5.48436379532833)*sech(r) + sech(r + 9.46098507279422) + 3.59521531836493)*sin(r)*sin(0.361068518529272*mu*nu*(nu + 5.57856998492368) + nu - 0.820610057737838*r*sin(theta) - theta + sin(mu)*tanh(nu) + sin(mu) + 9.59793972206312) + (0.00474892634022932*sin(nu) - 0.0113961399333476*sin(1.35184026490609*nu)*sin(1.83986261571299*mu + 3.81092634827696) + 0.144095459830625*sech(nu)*sech(0.891724521952723*r) + 0.087608369185607)*(-mu - nu - sech(mu - 0.0330031141374735) - sech(1.80751547275107*sin(nu)) + 7.19767174821328) - 0.0600448854189321*tanh(sin(nu)) + sech((nu + (0.365835914240071*mu + sech(r))*(0.0530060656699809*nu + 0.483950080970514) + tanh(mu) - 4.0323373076493)*(-2.11291650446067e-5*r*tanh(nu) + 2.34008056073503e-5*r + sech(mu*nu + 2.26262708691532) + 0.478949398217269)) + sech(nu**2*(0.960047907605333*nu - 3.35143838727204)*(nu - 13.7133605755259)) - 0.58023053996401
     ][2]
'''
