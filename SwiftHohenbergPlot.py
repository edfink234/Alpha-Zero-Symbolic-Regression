...
f =  [sin(r)*sin(theta), \
      ...
      -0.28580222883408**(r + 10)*(1.01 - sin(theta))*(167.620651926117*r**7.38905609893065 + 0.000105912014609458) + 0.833098208613807*0.999884875453817**(r**(17/4))*sqrt(1 - cos(r)**2)*sin(theta)  + 0.797073913381706][f_per_idx] \
...
Z = [np.sin(R)*np.sin(Theta), \
     ...
     -0.28580222883408**(R + 10)*(1.01 - np.sin(Theta))*(167.620651926117*R**7.38905609893065 + 0.000105912014609458) + 0.833098208613807*0.999884875453817**(R**(17/4))*np.sqrt(1 - np.cos(R)**2)*np.sin(Theta)  + 0.797073913381706][f_per_idx] \
...
formula_label = latex(round_floats(f, 3), mul_symbol='dot')
