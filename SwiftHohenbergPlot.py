...
if mu_equals_nu:
    ...
    f = [\
     ...
     (1.99057296709858 - 2.57998575900186*mu)*(0.0674740305895918*mu*nu + 1.81999731088334*nu + 0.0430890470354382) + 0.00124959990373851*(mu + 14.2640434126258)*(r + 0.000125980798570179)*sin(1.85685894548391*r) + (-0.367792888499773*nu*(-mu - 26.964582409306)*(mu - 0.769200575791721) - (nu + 0.699266134474235)*sech(nu))*(-0.000266768661794866*sech(mu) + sech(nu + 5.7713883766832) + 0.473721836313381) + (-3.87601678947507e-11*mu*r*(nu + r)*(0.0126593182464848*nu + 0.0126593182464848*(nu + r)*tanh(r) - 0.0126565998554621)*tanh(mu) + 2.87167637524512e-7)*(4.29056996560115*nu**3*(mu - 6.77658838178883)*(mu - 2.15869816483912)*(nu - 2.00865958265755)*(nu + sin(mu) - 9.01136328938673)*sin(nu) + 2*nu*(mu**2 + sin(r))*sin(mu) + 45.7681969232002*r*(sin(mu) + 1.31051705717967) + (0.0177992723027935*mu*sin(nu) + 27.5187507995053*sin(theta))*(sin(mu) + sin(nu))*sin(r + theta) + (tanh(mu) - 1.50278467988536)*(0.116063008354247*mu - 0.0864663653125089*nu + 4.51916746795576*r + sin(nu))*sech(sin(theta))) + 0.978336191182551*(-0.00254315751075186*mu - 0.168360096468398*nu + 3.55865077860287)*(-1.00040345067059*tanh(r) + tanh(1.00052394360405*r) - 0.743257667800217)*sin(r)*sin(0.361065038071669*mu*nu*(nu + 5.58479353570908) + 0.9068208922429*mu + 0.9813704859079*nu - 0.817111816864849*r*sin(theta) - theta + sin(mu) + tanh(mu)) + (mu - tanh(nu*sin(mu)) + 15.3619379301795*sech(nu))*(-1.3516580211404e-7*(mu + r)*sech(r) + (1.07495142676051e-6*r - 0.00551155603600697)*(nu**2*(nu - 2.93142060182891)*sech(nu**2) + sin(sin(mu)) + tanh(mu) - 0.568830518437333) + tanh(sech(nu) + sech(sin(mu)) + 1.6608540924677) - 0.98534409130279)*((tanh(mu) - 1.65558008319557)*tanh(sin(nu)) + tanh(mu) + sech(tanh(r)) - 0.596279940476664)*sin(1.58606395129335*nu + 1.10326157654256*sin(mu) - 0.169398271800054) + sech((sech(tanh(nu) + 2.73672693710822) + 0.183162507864409*sech(sin(r)))*(nu + (2.63425575624148 - 0.442142670357748*nu)*(0.545394436312422*mu - 4.67141358836136) - 0.482142720067475*sin(nu) + tanh(nu) + 3.59802539052655))
     ][2]
'''
