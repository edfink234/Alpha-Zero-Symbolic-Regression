...
Expression ExpressionParser::Parse(const char* buf,
                                   int & nerrors)
{
  curr_tok_ = PEV3PRINT; //`int PEV3PRINT = ';' (59)` in file `parser.h`
  ...
  while (*input_)
  {
    ...
    switch(curr_tok_)
    {
      case PEV3END: //`int PEV3END = 2` in file `parser.h`
        break;
      case PEV3PRINT: //`int PEV3PRINT = ';' (59)` in file `parser.h`
        continue;
      case PEV3RP: //`int PEV3RP = ')' (41)` in file `parser.h`
        if (no_of_functions_ == 0) error("primary expected, found", curr_tok_);
        else --no_of_functions_;
        continue;
      default:
        ret = expr(false); //addition and subtraction
    } // switch(curr_tok_)
  } // while (*input)
  delete input_;
  input_ = 0;
...
Expression ExpressionParser::term(const bool get)
{
  ...
  for (;;)
  {
    switch (curr_tok_)
    {
      case PEV3MUL: //`int PEV3MUL = '*' (42)` in file `parser.h`
        ret = ProductLink(ret, uminus(true));
        break;
      case PEV3DIV: //`int PEV3DIV = '/' (47)` in file `parser.h`
...
Expression ExpressionParser::expr(const bool get)
{
  ...
  for (;;)
  {
    switch (curr_tok_)
    {
      case PEV3PLUS: //`int PEV3PLUS = '+' (43)` in file `parser.h`
      case PEV3NLPLUS: //`int PEV3NLPLUS = '|' (124)` in file `parser.h`
...
Token_value ExpressionParser::get_token()
{
  ...
  switch(ch)
  {
    ...
    case '^':
    case '#':
      return curr_tok_ = Token_value(ch); //`Token_value` enum in file `parser.h`
    case '0':
    case '1':
    case '2':
    case '3':
