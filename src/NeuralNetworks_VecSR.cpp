    ...
    float complete_status(bool cache = true)
    {
        ...
        else
        {
            if (this->is_primary)
            {
                ...
                float MSE = this->srnn.train(data.rows, data.labels, this->epochs);
//                std::cout << "MSE = " << MSE << '\n';
        ...
        const std::pair<int, float>& mse_thresh = {} /*number of individuals (.first) that must have an mse below (.second) before returning (.first) best individuals*/)
{
    ...
    auto func = [&](int thread_num)
    {
        ...
        if (seed_individuals.size())
        {
            for (int i = init_population*thread_num; i < 2*init_population*thread_num; i++)
            {
                individuals.push_back(seed_individuals[i%seed_individuals.size()]);
            }
        }
...
//                assert(score >= 0); //make sure it's a valid math expression
                if (score < 0)
                {
                    ...
                    //delta_w_t_k_ada_delta gamma * delta_w_t_k_ada_delta beta_2 sqrt beta_2 sqrt t + ^ / + 
                }
            }
            else
            {
            ...
            if (!std::isnan(score))
            {
                ...
                printf("thread = %d, i = %d, init_population = %d\n", thread_num, i, init_population);
                std::cout << "individuals[" << i-1 << "].first = " << individuals[i-1].first << '\n';
            }
            else
            {
                if (count_fails++ > numFailsBeforeExit && i >= 2)
                {
                    break;
                }
                //std::cout << "Thread " << i << " fails = " << count_fails++ << '\n';
            }
        }
...
//            }
            ...
            individuals.resize(std::min(init_population, static_cast<int>(individuals.size()))); //keep only the best `init_population` individuals.
        }
	if (!passedMSE())
	{
		//push_back the individuals into best_individuals
		std::scoped_lock str_lock(Board::thread_locker);
		for (int i = 0; i < individuals.size(); i++)
		{	
			if (!std::isnan(individuals[i].second))
				best_individuals[individuals[i].second] = individuals[i].first;
		}
	}
    };
	...
	//if (best_individuals.size() > num_threads * init_population)
	{
	    // 1. Copy elements from the unordered_map to a vector of pairs
	    std::vector<std::pair<float, std::vector<std::string>>> sorted_elements(
		best_individuals.begin(), best_individuals.end()
	    );

	    // 2. Sort the vector by the float key from highest to lowest
	    // We use a lambda function to compare the .first element of the pairs
	    std::sort(sorted_elements.begin(), sorted_elements.end(), 
		      [](const auto& a, const auto& b) {
			  return a.first > b.first; 
		      });

	    // 3. Clear the original map
	    best_individuals.clear();

	    // 4. Insert the top (num_threads * init_population) elements back into the map
	    size_t target_size = num_threads * init_population;
	    for (size_t i = 0; i < target_size && i < sorted_elements.size(); ++i)
	    {
		best_individuals.insert(std::move(sorted_elements[i]));
	    }
	}
    ...
    if (mse_thresh.first > 0)
    {
        ...
        assert(allIndividuals.size() >= 0);
	printf("allIndividuals.size() = %lu\n",allIndividuals.size());
	if (allIndividuals.size() > mse_thresh.first)
	{	
...
int main()
{
    ...
    //    system((std::string("cat ")+tempMSE_filename).c_str());
        ...
        allIndividuals = GP(my_temp_test_data,
               ...
               "postfix",
               true /*cache*/,
               15000 /*time to run the algorithm in seconds*/,
               100 /*size of initial population(s)*/,
               1 /*num threads*/,
...
//x x - => x-x                 0
//x x + => x+x     x x +
//scp -i ~/Downloads/MyNNWeightUpdateVM_key.pem MLP_Vec.cpp MLP_Vec.h NeuralNetworks_VecSR.cpp azureuser@52.229.230.145:~
