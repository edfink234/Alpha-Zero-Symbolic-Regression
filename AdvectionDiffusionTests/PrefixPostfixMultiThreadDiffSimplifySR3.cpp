    ...
    Board(std::vector<float> (*diffeq)(Board&), bool primary = true, int n = 3, const std::string& expression_type = "prefix", std::string fitMethod = "PSO", int numFitIter = 1, std::string fitGradMethod = "naive_numerical", const Eigen::MatrixXf& theData = {}, bool visualize_exploration = false, bool cache = false, bool const_tokens = false, float isConstTol = 1e-1f, bool const_token = false, std::string boundary_condition_type = "none", std::string initial_condition_type = "none") : gen{rd()}, vel_dist{-1.0f, 1.0f}, pos_dist{0.0f, 1.0f}, num_fit_iter{numFitIter}, fit_method{fitMethod}, fit_grad_method{fitGradMethod}, is_primary{primary}
    {
        ...
        if (is_primary)
        {
            std::call_once(initialization_flag, [&]()
            {
                ...
                {
                    Board::__other_tokens.push_back(std::to_string(Board::data[i].minCoeff())); //add smallest element
                    Board::__other_tokens.push_back(std::to_string(Board::data[i].maxCoeff())); //add largest element
                }
                ...
                int fmm_idx = ops_plus_features + 1;
                for (int i = fmm_idx; i < fmm_idx + feature_mins_maxes_start_val; i++)
                {
                    Board::__other_tokens_float.push_back(i);
                }
                for (int i = fmm_idx + feature_mins_maxes_start_val, j = 0; i < fmm_idx + feature_mins_maxes_end_val; i+=2, j++)
                {
                    Board::__other_tokens_float.push_back(i);
                    Board::__other_tokens_float.push_back(i+1);
                    feature_mins_maxes[Board::__input_vars[j]] = std::make_pair(i, i+1);
                }
                fmm_idx += feature_mins_maxes_end_val;
                for (int i = fmm_idx; i <= Board::action_size; i++)
                {
                    assert(false || const_token);
                    Board::__other_tokens_float.push_back(i);
                }
                
                Board::__tokens_dict.clear();
                Board::__tokens_inv_dict.clear();
                ...
                {
                    assert(i <= Board::__tokens_float.back());
...
//            std::cout << "pieces[i] = " << pieces[i] << '\n';
            ...
            if (std::find(Board::__operators_float.begin(), Board::__operators_float.end(), pieces[i]) == Board::__operators_float.end()) //not an operator, i.e., a leaf
            {
                if (token.substr(0,5) == "const")
                {
                    stack.push(Eigen::VectorXf::Ones(Board::data.numRows())*params((!this->const_token ? (const_count++) : const_idx)));                    
                }
                else if (token == "0")
                {
...
//x0 -> x, x1 -> y, x2 -> t
std::vector<float> TwoDAdvectionDiffusion_1(Board& x)
{
    std::vector<float> result;
    result.reserve(100);
    std::vector<int> grasp;
...
//x0 -> x, x1 -> y, x2 -> t
std::vector<float> TwoDAdvectionDiffusion_2(Board& x)
{
    std::vector<float> result;
    result.reserve(100);
    std::vector<int> grasp;
...
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "Simulated Annealing & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
...
void GP(std::vector<float> (*diffeq)(Board&), const Eigen::MatrixXf& data, int depth = 3, std::string expression_type = "prefix", std::string method = "LevenbergMarquardt", int num_fit_iter = 1, const std::string& fit_grad_method = "naive_numerical", bool cache = true, double time = 120, unsigned int num_threads = 0, bool const_tokens = false, float isConstTol = 1e-1f, bool const_token = false, std::string boundary_condition_type = "none", std::string initial_condition_type = "none")
{
    ...
    auto func = [&diffeq, &depth, &expression_type, &method, &num_fit_iter, &fit_grad_method, &data, &cache, &start_time, &time, &max_score, &sync_point, &best_expression, &orig_expression, &best_expr_result, &orig_expr_result, &const_tokens, &isConstTol, &const_token, &best_MSE, &boundary_condition_type, &initial_condition_type]()
    {
        ...
        constexpr int init_population = 200;
...
//    std::cout << "Best diff result = " << best_expr_result << '\n';
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "GP & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
}
...
//    std::cout << "Best diff result = " << best_expr_result << '\n';
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "PSO & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
}
...
//    std::cout << "Best diff result = " << best_expr_result << '\n';
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "Concurrent MCTS & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
}
...
//    std::cout << "Best diff result = " << best_expr_result << '\n';
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "MCTS & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
}
    ...
    auto func = [&diffeq, &depth, &expression_type, &method, &num_fit_iter, &fit_grad_method, &data, &cache, &start_time, &time, &max_score, &sync_point, &best_expression, &orig_expression, &best_expr_result, &orig_expr_result, &const_tokens, &isConstTol, &const_token, &best_MSE, &boundary_condition_type, &initial_condition_type]()
    {
        ...
        while (timeElapsedSince(start_time) < time)
        {
            while ((score = x.complete_status()) == -1)
            {
                ...
                if (!temp_sz)
                {
                    for (const auto& i: x.pieces)
                    {
                        std::cout << Board::__tokens_dict[i] << ' ';
                    }
                    puts("");
                }
                assert(temp_sz);
...
//    std::cout << "Best diff result = " << best_expr_result << '\n';
//    std::cout << "Best expression (original format) = " << orig_expr_result << '\n';
    std::cout << "Random Search & " << depth << " & " << expression_type << " & " << best_MSE << " & " << std::boolalpha << const_tokens <<
    " & " << std::boolalpha << const_token << '\n';
}
...
int main()
{
    ...
    for (int depth = 1; depth < 31; depth++)
    {
        ...
        for (int notation = 0; notation < num_notations; notation++)
        {
            ...
            //const_tokens: true, const_token: true
            Board::expression_dict.clear();
            RandomSearch(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
            Board::expression_dict.clear();
            MCTS(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
            Board::expression_dict.clear();
            ConcurrentMCTS(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
            Board::expression_dict.clear();
            PSO(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
            Board::expression_dict.clear();
            GP(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
            Board::expression_dict.clear();
            SimulatedAnnealing(TwoDAdvectionDiffusion_1 /*differential equation to solve*/, data1 /*data used to solve differential equation*/, depth /*fixed depth of generated solutions*/, notations[notation] /*expression representation*/, "PSO" /*fit method if expression contains const tokens*/, 5 /*number of fit iterations*/, "autodiff" /*method for computing the gradient*/, true /*cache*/, time /*time to run the algorithm in seconds*/, 0 /*num threads*/, true /*`const_tokens`: whether to include const tokens {0, 1, 2, 4}*/, 5.0e-1 /*threshold for which solutions cannot be constant*/, true /*whether to include "const" token to be optimized, though `const_tokens` must be true as well*/, "AdvectionDiffusion2D_1" /*boundary condition type*/, "AdvectionDiffusion2D" /*initial condition type*/);
        }
    }
...
//g++ -Wall -std=c++20 -o PrefixPostfixMultiThreadDiffSimplifySR3 PrefixPostfixMultiThreadDiffSimplifySR3.cpp -O2 -I/opt/homebrew/opt/eigen/include/eigen3 -I/opt/homebrew/opt/eigen/include/eigen3 -I/Users/edwardfinkelstein/LBFGSpp -ffast-math -ftree-vectorize -L/opt/homebrew/Cellar/boost/1.84.0 -I/opt/homebrew/Cellar/boost/1.84.0/include -march=native

//g++ -Wall -std=c++20 -o PrefixPostfixMultiThreadDiffSimplifySR3 PrefixPostfixMultiThreadDiffSimplifySR3.cpp -g -I/opt/homebrew/opt/eigen/include/eigen3 -I/opt/homebrew/opt/eigen/include/eigen3 -I/Users/edwardfinkelstein/LBFGSpp -L/opt/homebrew/Cellar/boost/1.84.0 -I/opt/homebrew/Cellar/boost/1.84.0/include -march=native
